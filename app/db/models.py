...
from sqlalchemy import Column, String, Boolean, DateTime, Numeric, Date, BigInt, Enum, ForeignKey
...
class Instrument(Base):
    ...
    instrument_type = Column(Enum('ETF', 'MF', name='instrument_type_enum'), nullable=False)
...
class Transaction(Base):
    ...
    transaction_type = Column(Enum('BUY', 'SELL', 'DIVIDEND', 'SIP', name='transaction_type_enum'), nullable=False)
...
class BacktestResult(Base):
    ...
    parameters = Column(postgresql.JSONB, nullable=True)
    equity_curve = Column(postgresql.JSONB, nullable=True)
    benchmark_curve = Column(postgresql.JSONB, nullable=True)
    xirr = Column(Numeric(10, 4), nullable=True)
    summary_stats = Column(postgresql.JSONB, nullable=True)
    created_at = Column(DateTime, default=datetime.utcnow)
...
class SymbolCache(Base):
    ...
    symbol = Column(String, unique=True, index=True, nullable=False)
    name = Column(String, nullable=False)
    instrument_type = Column(Enum('ETF', 'MF', 'INDEX', name='symbol_instrument_type_enum'), nullable=False)
